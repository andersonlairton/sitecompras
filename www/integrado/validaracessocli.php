...
<head>
   ...
   $Senha=$_POST["Senha"]; 
    ...
    $SQL = sprintf("SELECT * FROM tb_cliente WHERE CNPJ = '$CNPJ' AND Senha = '$Senha'");
    ...
    if ($row> 0){ //condição,se tiver mais qe um registo ele acessa o banco
      session_start();
       ...
       //  $_SESSION["mycnpj"] = $CNPJ["CNPJ"];
        header("location:telaCli.php");
        $SQL2 = sprintf("SELECT NOME FROM tb_forncedor_npj WHERE CNPJ = $CNPJ");
